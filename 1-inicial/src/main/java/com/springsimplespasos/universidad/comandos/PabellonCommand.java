...
public class PabellonCommand implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		Iterable<Pabellon> cPaellon = servicio
				.buscarPabellonesPorLocalidad(ObjetosDummy.getPabellonUno().getDireccion().getLocalidad());
		...
		Optional<Pabellon> oPabellon = servicio
				.findPabellonByNombreIgnoreCase(ObjetosDummy.getPabellonDos().getNombre());
		...
		Iterable<Pabellon> iPabellon = servicio.findPabellonsByFechaAltaBetween(LocalDateTime.now().minusHours(2L),
				LocalDateTime.now());
