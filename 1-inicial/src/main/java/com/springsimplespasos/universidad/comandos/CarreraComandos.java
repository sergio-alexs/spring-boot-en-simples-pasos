...
public class CarreraComandos implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		Iterable<Carrera> iCarreras = servicio
				.findCarrerasByCantidadAniosAfter(ObjetosDummy.getCarreraIngSis().getCantidadAnios());
		 ...
		 * Carrera licYoga = new Carrera(null, "Licenciatura en Yoga", 25, 3); Carrera
		 * licRecursos = new Carrera(null, "Licenciatura en Recursos Humanos - RRHH",
		 * 33, 3);
		 * 
		 * servicio.save(ingSistemas); servicio.save(ingIndustrial);
		 * servicio.save(ingAlimentos); servicio.save(ingElectronica);
		 * servicio.save(licSistemas); servicio.save(licTurismo);
		 * servicio.save(licYoga); servicio.save(licRecursos);
		 */

		/*
		 * List<Carrera> carreras =
		 * (List<Carrera>)servicio.findCarrerasByNombreContains("Sistemas");
		 * carreras.forEach(System.out::println);
		 * 
		 * List<Carrera> carrerasIgnoreCase1 = (List<Carrera>)
		 * servicio.findCarrerasByNombreContainsIgnoreCase("SISTEMAS"); List<Carrera>
		 * carrerasIgnoreCase2 = (List<Carrera>)
